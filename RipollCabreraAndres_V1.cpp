...

{
  ...
  m_vPosition.x = 0.0; //This is the variable where X coordinate should be calculated
  m_vPosition.y = 0.0; // This is the variable where Y coordinate should be calculated
  m_fOrientation  = 0.0; //This is the variable where orientation  shouldbe calculated
...
void CTestCompassController::SimulationStep(unsigned n_step_number, double f_time, double f_step_interval)
{
   ...
   * IMPORTANT!!: The code must exclusively depend on "encoder" variable */
  /* Update m_vPosition.x, m_vPosition.y and m_fOrientation variables */
    
   m_vPosition.x += (encoder[0]+encoder[1])/2*cos(m_fOrientation);
   m_vPosition.y += (encoder[0]+encoder[1])/2*sin(m_fOrientation);
   m_fOrientation += (encoder[1]-encoder[0])/fWheelsDistance;
   if(m_fOrientation<0){
    m_fOrientation += 2*M_PI;
   }
   if(m_fOrientation>2*M_PI){
    m_fOrientation -= 2*M_PI;                 
   }
